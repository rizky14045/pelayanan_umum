...
<div class="container">
    <div class="row">
        <div class="col-md-3">
            @include('front.baru.profile.partials.sidebar', ['active' => 'ruangan'])
        </div>

        <div class="col-md-9">
			<div class="kpi-toolbar">
				<div class="kpi-toolbar-search">
					<input id="search-ruangan" class="form-control" placeholder="Cari nama acara ..."/>
				</div>
			</div>

			<div id="kpi-list">
				@forelse($pagination as $pemesananRuangan)
					@php
					...
					@endphp
					<div class="kpi-card" data-acara="{{ $pemesananRuangan->nama_acara }}">
						...
						<div class="kpi-card-body">
							<div class="kpi-card-row">
								<div class="kpi-card-title-wrap">
									...
									<div class="kpi-card-meta">
										<span><i class="fa fa-calendar"></i> {{ $pemesananRuangan->tanggal }} &ndash; {{ $pemesananRuangan->tanggal_selesai }}</span>
										<span><i class="fa fa-clock-o"></i> {{ date("H:i",$pemesananRuangan->waktu_awal)}} &ndash; {{ date("H:i",$pemesananRuangan->waktu_akhir)}}</span>
										<span><i class="fa fa-building"></i> {{ $pemesananRuangan->ruang['nama_ruang'] ?? '-' }}</span>
										<span><i class="fa fa-users"></i> {{ $pemesananRuangan->jumlah_peserta }} peserta</span>
									</div>
								</div>
								<div class="kpi-card-actions">
									<div class="kpi-action-buttons">
										@if($pemesananRuangan->status_pj == 'Pending')
											<a class="btn btn-sm btn-danger delete-button" href="#" data-id="{{$pemesananRuangan->id}}"><i class="fa fa-trash"></i> Delete</a>
										@endif
									</div>
							...
							<div class="kpi-card-detail">
								<div class="kpi-detail-grid">
									<div class="kpi-detail-item"><span class="kpi-detail-label">No Pemesanan</span><span class="kpi-detail-value">{{ $pemesananRuangan->no_pemesanan_ruangan }}</span></div>
									<div class="kpi-detail-item"><span class="kpi-detail-label">Nama Pemesan</span><span class="kpi-detail-value">{{ $pemesananRuangan->nama_pemesan }}</span></div>
									...
									<div class="kpi-detail-item"><span class="kpi-detail-label">Attachment</span><span class="kpi-detail-value">
										@if ($pemesananRuangan->attachment)
				...
				@empty
					<div class="kpi-empty">Belum ada pemesanan ruangan.</div>
				@endforelse
			</div>
...
<script>
	...
	function kpiFilteredCards() {
		var keyword = $('#search-ruangan').val().toLowerCase();
		return $('#kpi-list .kpi-card').filter(function () {
			var acara = ($(this).data('acara') || '').toString().toLowerCase();
			return acara.indexOf(keyword) !== -1;
		});
	}
